...
import sqlite3
class Ui_sup(QtWidgets.QDialog):
    def supprimerDialoq_accepter(self):
        try:
            db = self.open_connection()
            connection_s1 = db.cursor()
            print(self.comboBox.currentText())
            connection_s1.execute(f"""DELETE FROM Conge WHERE Id = {int(self.comboBox.currentText())}""")
            db.commit()
            db.close()
    ...
    def Refresh_Accept_Supp(self):
        ...
        for v in self.result:
            self.comboBox.addItem(str(v[0]))
        db.close()
    def __init__(self,Nom,Prenom):
            super().__init__()
            loadUi("supprimer_Acceptation.ui",self)
            ...
            self.Refresh_Accept_Supp()
            self.pushButton.clicked.connect(self.supprimerDialoq_accepter)
